...
function skyPropsHandler({ temperature, skyStatus, rainProbablity }) {
    // Display filter darkens the image and props depending on sky state and rain probability
    // Set classes based on that data
    const displayFilterClasses = setDisplayFilterClasses(skyStatus, rainProbablity);
  
    // Set clouds classes depending on sky state and rain probability
    const cloudsClasses = setCloudsClasses(skyStatus, rainProbablity);
    ...
    const rainClasses =
    rainProbablity.includes('probable') && skyStatus !== 'clear'
        ? 'rain'
        : 'rain rain-hidden';
  ...
  function setDisplayFilterClasses(skyStatus, rainProbablity) {
    // Add each class as a string to the array
    ...
    if (rainProbablity.includes('probable')) displayFilter.push('display-filter-darker');
  
    // Return the array as a string
    return displayFilter.join(' ');
  }
  
  function setCloudsClasses(skyStatus, rainProbablity) {
    let cloud1;
    let cloud2;
    let cloud3;
    let cloud4;
    ...
    if (rainProbablity.includes('probable') && skyStatus !== 'clear') {
